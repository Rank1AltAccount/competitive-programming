...
int getMoneySpent(const vector<int>& keyboards, const vector<int>& drives, const int b) {
    int moneySpent = -1; // -1 means you can't buy both items
    
    // Compare pairs of each keyboard and drive
    // and check if their cost exceeds the previous most expensive pair
    // within a given budget
    for (int i = 0; i < keyboards.size(); i++) {
        for (int j = 0; j < drives.size(); j++) {
            int cost = keyboards[i] + drives[j];
            if (cost > moneySpent and cost <= b) {
                moneySpent = cost;
            }
        }
    }
